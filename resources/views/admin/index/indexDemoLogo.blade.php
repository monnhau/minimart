...
<body >
@else
<body style="background: {{$MauNenProvider->value_char}} !important">
@endif

    <div class="verify_color" style="height: 53px;background: #333;color: #fff;margin-bottom: 10px;text-align: center;line-height: 50px;">
		 <span>Bạn muốn hoàn thành việc chọn logo?</span>
		 <a style="background: green;padding: 5px 15px;margin-right: 5px;" href="{{route('admin.valueOption.updateLogoReal')}}">OK</a>
		 <a style="background: red;padding: 5px 15px;margin-right: 5px;" href="{{route('admin.valueOption.updateLogoCancel')}}">Hủy</a>
    </div>
	<div class="container">
		<div class="header">
        @php
        $logoDemo = App\ValueOption::getItemByName("logo_demo");
        @endphp
        @if($logoDemo == null)
        <img src="{{$LogoUrl}}{{$DefaultImg}}" >
        @else
        <img src="{{$LogoUrl}}{{$logoDemo->value_char}}" >
        @endif
		...
		<div class="content" style="background: ghostwhite !important">
